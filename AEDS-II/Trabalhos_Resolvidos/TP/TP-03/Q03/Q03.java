...
class Lista {
    ...
    public void mostrar() {
        for (int i = 0; i < tamanho; i++) {
            series[i].printClass();
            //MyIO.println(series[i].getLanguage());
        }
    }
    
    public void ordenarInsercaoAlfabetico() {
        int n = tamanho;
        for (int i = 1; i < n; i++) {
            Serie tmp = series[i].clone();
            movimentacoes++;
            int j = i - 1;
            comparacoes++;
            while ((j >= 0) && (series[j].getName().compareTo(tmp.getName()) > 0)) {
                series[j + 1] = series[j].clone();
                movimentacoes++;
                j--;
            }
            series[j + 1] = tmp.clone();
            movimentacoes++;
        }
    }
    ...
    public void ordenarInsercao() {
        ...
        for (int i = 1; i < n; i++) {
            ...
            int j = i - 1;
            comparacoes++;
            while ((j >= 0) && (series[j].getLanguage().compareTo(tmp.getLanguage()) > 0)) {
                series[j + 1] = series[j].clone();
                movimentacoes++;
                j--;
            }
...
public class Q03 {
    ...
    public static void main(String[] args) {
        ...
        lista.ordenarInsercaoAlfabetico();
        ...
        Arq.openWrite("728738_insercao.txt");
