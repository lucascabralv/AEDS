...
class Lista {
    ...
    public void mostrar() {
        for (int i = 0; i < tamanho; i++) {
            series[i].printClass();
            // Print para teste
            //MyIO.println(series[i].getSeasons() + " --- " + series[i].getName());
        }
    }
    ...
    private void ordenar(int inicio) {
        int i = inicio + 1;
        while (i < tamanho && series[inicio].getSeasons() == series[i].getSeasons()) {
            i++;
        }
        i--;
        if (i < tamanho) {
            bubblesortName(inicio, i+1);
            ordenar(i + 1);
        }
    }
    ...
    private void bubblesortName(int inicio, int fim) {
        for (int i = (fim - 1); i > inicio; i--) {
            for (int j = inicio; j < i; j++) {
                if (series[j].getName().compareTo(series[j+1].getName()) > 0) {
                    swap(j, j + 1);
                }
            }
        }
    ...
    public void bubblesort() {
        for (int i = (tamanho - 1); i > 0; i--) {
            for (int j = 0; j < i; j++) {
                comparacoes++;
                if (series[j].getSeasons() > series[j + 1].getSeasons()) {
                    swap(j, j + 1);
                }
            }
        }
...
public class Q08 {
    ...
    public static void main(String[] args) {
        ...
        Arq.openWrite("728738_bolha.txt");
