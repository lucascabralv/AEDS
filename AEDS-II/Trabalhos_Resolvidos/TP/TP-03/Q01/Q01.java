...
class Lista {
    ...
    public void mostrar() {
        for (int i = 0; i < tamanho; i++) {
            series[i].printClass();
            //MyIO.println(series[i].getCountry());
        }
    }
    ...
    public void ordenarSelecao() {
        ...
        for (int i = 0; i < (n - 1); i++) {
            int menor = i;
            for (int j = (i + 1); j < n; j++) {
                if (series[menor].getCountry().compareTo(series[j].getCountry()) > 0) {
                    menor = j;
                } else if(series[menor].getCountry().equals(series[j].getCountry())){
...
public class Q01 {
    ...
    public static void main(String[] args) {
        ...
        lista.ordenarSelecao();

        //End timer
        long fim = new Date().getTime();
        double tempo = (fim - inicio) / 1000.0;
        Arq.openWrite("728738_selecao.txt");
