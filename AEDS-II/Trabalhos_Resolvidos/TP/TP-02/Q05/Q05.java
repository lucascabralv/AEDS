...
class Serie {
    ...
    public String toString(){
        String resp = this.name + " " + this.format + " " + this.duration + " " + 
                      this.country + " " + this.language + " " + 
                      this.broadcaster + " " + this.streaming + " " + this.seasons + " " + 
                      this.episodes;
        return resp;
    }
...
class Lista {
    ...
    public Serie removerFim() throws Exception {
        ...
        //MyIO.println("Serie removida: " + series[tamanho].getName() + " -> " + tamanho);
...
public class Q05 {
    ...
    public static void lerComandos(String linha, Lista lista) throws Exception{
        String comando = linha.substring(0,2);
        String serieArquivo;
        int pos;
        switch(comando){
            //Inserir Início
            case "II":{
                serieArquivo = linha.substring(3, linha.length());
                lista.inserirInicio(criaSerie(serieArquivo));
                break;
            }
            //Inserir Fim
            case "IF":{
                serieArquivo = linha.substring(3, linha.length());
                lista.inserirFim(criaSerie(serieArquivo));
                break;
            }
            //Inserir em Posição
            case "I*":{
                pos = Integer.parseInt(linha.substring(3, 5));
                serieArquivo = linha.substring(6, linha.length());
                lista.inserir(criaSerie(serieArquivo), pos);
                break;
            }
            //Remover Início
            case "RI":{
                Serie retorno = lista.removerInicio();
                MyIO.println("(R) " + retorno.getName());
                break;
            }
            //Remover Fim
            case "RF":{
                Serie retorno = lista.removerFim();
                MyIO.println("(R) " + retorno.getName());
                break;
            }
            //Remover em Posição
            case "R*": {
                pos = Integer.parseInt(linha.substring(3, 5));
                Serie retorno = lista.remover(pos);
                MyIO.println("(R) " + retorno.getName());
                break;
            }
        }
